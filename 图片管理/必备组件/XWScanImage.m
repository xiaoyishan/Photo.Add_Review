...
static CGRect frame_first;
static UIImageView *fullImageView;

+ (void)scanBigImageWithImageView:(UIImageView *)imageView view:(UIView*)view{
    if ([view.window viewWithTag:111]) {
        return ;
    }
    frame_first = [imageView convertRect:imageView.bounds toView:[UIApplication sharedApplication].keyWindow];
    fullImageView=[[UIImageView alloc] initWithFrame:CGRectMake(0, 0, Width, Height)];
    fullImageView.backgroundColor=[UIColor blackColor];
    fullImageView.userInteractionEnabled=YES;
    fullImageView.tag = 111;
//    [fullImageView addGestureRecognizer:[[UITapGestureRecognizer alloc] initWithTarget:self action:@selector(actionTap2:)]];
    ...
    if (![fullImageView superview]) {
        ...
        [view.window addSubview:fullImageView];
        fullImageView.frame=frame_first;
        [UIView animateWithDuration:0.5 animations:^{
            fullImageView.frame=CGRectMake(0, 0, Width, Height);
